...
# External modules (project-internal)
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            if resp.status_code < 400:
                ...
                return  # << If connection successful, CONTINUE
            else:
                print(f" Warning: Server responded with status {resp.status_code} at {url}")
                return  # << Server is alive, so continue
            ...
            if attempt < retries:
                ...
                sys.exit(1)  # << Only exit after all retries
...
def main() -> None:
    ...
    for i in range(1, 11):
        parser.add_argument(f"--api{i}", action="store_true", help=f"Only perform API{i}-audit")
    ...
    print(f"\n Loading Swagger: {args.swagger}")
    bola = BOLAAuditor(sess)
    spec = bola.load_swagger(args.swagger)
    if not spec:
        sys.exit("Swagger could not be loaded or is empty.")
    endpoints = bola.get_object_endpoints(spec)
    print(f" Swagger loaded – {len(endpoints)} endpoints found")
        ...
        with ThreadPoolExecutor(max_workers=args.threads) as ex:
            ...
            for fut in futures:
                try:
                    ...
                    print(f" BOLA error: {e}")
        report = bola.generate_report(bola_results)
        (output_dir / "api_bola_report.txt").write_text(report, "utf-8")
        print(f"✅ API1 complete – {sum(1 for r in bola_results if getattr(r, 'is_vulnerable', False))} vulnerabilities")
    ...
    if 2 in selected:
        ...
        print(f"✅ API2 complete – {len(auth_issues)} issues")
    ...
    if 3 in selected:
        ...
        for issue in prop_issues:
            print(f"-> Property issue found: {issue.get('description', 'Unknown problem')} @ {issue.get('endpoint', 'Unknown endpoint')}")
        (output_dir / "api_property_report.txt").write_text(pa.generate_report(), "utf-8")
        print(f"✅ API3 complete – {len(prop_issues)} issues")
    ...
    if 4 in selected:
        ...
        print(f"✅ API4 complete – {len(res_issues)} issues")
    ...
    if 5 in selected:
        ...
        for issue in authz_issues:
            print(f"-> Authorization issue: {issue.get('description', 'Unknown problem')}")
        (output_dir / "api_admin_access_report.txt").write_text(za.generate_report(), "utf-8")
        print(f"✅ API5 complete – {len(authz_issues)} issues")
    ...
    if 6 in selected:
        ...
        print(f"✅ API6 complete – {len(biz_issues)} issues")
    ...
    if 7 in selected:
        ...
        if ss_eps:
            ...
            print(f"✅ API7 complete – {len(ssrf_issues)} issues")
        else:
            print("ℹ️ No SSRF endpoints found")
    ...
    if 8 in selected:
        ...
        print(f"✅ API8 complete – {len(misconf_issues)} issues")
    ...
    if 9 in selected:
        ...
        print(f"✅ API9 complete – {len(inv_issues)} issues")
    ...
    if 10 in selected:
        ...
        print(f"✅ API10 complete – {len(safe_issues)} issues")
