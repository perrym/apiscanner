    ...
    try:
        for op in oas_iter_ops(spec):
            eps.append({
                "path": op["path"], 
                "method": op["method"], 
                "operationId": op.get("operation",{}).get("operationId") or f"{op['method']}_{op['path'].strip('/').replace('/','_')}", 
                "tags": op.get("operation",{}).get("tags", [])
            })
...
def main() -> None:
    ...
    if getattr(args, 'plan_only', False) or getattr(args, 'plan_then_scan', False):
        plan_requests(spec, base, csv_path=None, rewrites=getattr(args, 'rewrite', []), 
                     disable_sanitize=getattr(args, 'no_sanitize', False), normalize_version=getattr(args, 'normalize_version', False))
    ...
    if getattr(args, 'verify_plan', False):
        oks, fails, total = verify_plan(args, sess, spec, base, csv_path=None, 
                                       ...
                                       disable_sanitize=getattr(args, 'no_sanitize', False))
    ...
    if 5 in selected_apis:
        ...
        za = AuthorizationAuditor(session=sess, base_url=args.url, spec=spec, 
                                flow=getattr(args, "flow", "none"), logger=logger)
    ...
    if 6 in selected_apis:
        ...
        bf = BusinessFlowAuditor(session=sess, base_url=args.url, swagger_spec=spec, 
