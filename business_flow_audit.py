...
class BusinessFlowAuditor:
    ...
    def __init__(self, base_url: str, session: Optional[requests.Session]=None, *, concurrency: int=3, timeout: int=6, swagger_spec: Optional[dict]=None, show_progress: bool=True) -> None:
    ...
    def _run_tests_for_flow(self, flow: Flow):
        self._tw(f"[FLOW] {flow['method']} {flow['url']}")
    ...
    def _send(self, flow: Flow, *, body: Optional[Dict[str, Any]]=None, override_method: Optional[str]=None) -> Optional[requests.Response]:
        ...
        try:
            ...
            if method == 'GET':
                return self.session.get(url, headers=headers, params=params, timeout=current_timeout)
            return self.session.request(method=method, url=url, headers=headers, params=params, json=final_body, timeout=current_timeout)
        except requests.exceptions.ReadTimeout:
            self._tw('Timout ..')
            return None
        except requests.exceptions.ConnectionError:
            self._tw('Connection error...')
            return None
        except requests.exceptions.RequestException as e:
            self._tw('[ERROR] Request failed for {url}: {e}')
            return None
