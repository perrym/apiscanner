...
class MisconfigurationAuditorPro:
    ...
    def _security_header_analyzer(self, ep: Endpoint, payload: str, resp: requests.Response, dur: float) -> Optional[Finding]:
        ...
        if missing:
            return self._build_finding(ep, payload, resp, dur, f"Missing security headers: {', '.join(missing)}", "Medium")
        return None

    def _reflected_ssrf_analyzer(self, ep: Endpoint, payload: str, resp: requests.Response, dur: float) -> Optional[Finding]:
        if payload in resp.text:
            return self._build_finding(ep, payload, resp, dur, "Payload reflected (possible reflected SSRF)", "High")
        return None

    def _blind_ssrf_analyzer(self, ep: Endpoint, payload: str, resp: requests.Response, dur: float) -> Optional[Finding]:
        if dur > 3.0:
            return self._build_finding(ep, payload, resp, dur, f"Slow response ({dur:.2f}s) – possible blind SSRF", "Medium")
        return None
    ...
    def _build_finding(self, ep: Endpoint, payload: str, resp: requests.Response, dur: float, reason: str, severity: str) -> Finding:
        return {
            ...
            "response_sample": resp.text[:200] if resp.text.strip() else "(No body received)",
        }
    ...
    def generate_report(self, fmt: str = "markdown") -> str:
        if not self._findings:
            return "No misconfigurations or vulnerabilities found."
        if fmt == "json":
            return json.dumps(self._findings, indent=2)

        out: List[str] = [
            "# API Security Audit – Misconfigurations & SSRF", 
            f"📅 Scan performed on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
            f"🌐 Target: {self.base_url}",
            f"⚙️ Configuration: {self.concurrency} threads, timeout {self.timeout}s",
            "\n---",
        ]
        ...
        for sev in severity_order:
            ...
            if not issues:
                continue
            out.append(f"\n## {sev_icons.get(sev, '')} {sev} Findings\n")
            for i in issues:
                out.append(f"### {i['endpoint']}")
                out.append(f"- **Vulnerability**: {i['result']}")
                out.append(f"- **Payload**: `{i['payload']}`")
                out.append(f"- **Response time**: {i['response_time']}")
                out.append(f"- **Status code**: {i['response_status']}")
                out.append(f"- **Important headers**:")
                out.append(f"  ```json\n{json.dumps(i.get('response_headers', {}), indent=2)}\n```")
                out.append(f"- **Response sample**:")
                out.append(f"  ```\n{i.get('response_sample', '')}\n```\n")

        return "\n".join(out)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Audit OWASP API8 – Misconfigurations & SSRF")
