...
A robust "safe consumption of third-party APIs" auditor.

Highlights:
- Strong false-positive controls (JSON parse, WAF/generic 4xx/5xx, timeouts)
- Conservative SSRF, CRLF, HPP, header manipulation, GraphQL checks
- Per-host throttling with bounded concurrency, large connection pool
- Fast triage mode (few payloads first, deepen only on signal)
- Clean reporting + dedup + RFC7807 downgrades
"""
...
# Silence TLS warnings for verify=False calls (e.g., kube probe)
...
def _headers_to_list(headerobj):
    """
    Return all (key, value) header pairs including duplicates,
    whether it's a dict or HTTPHeaderDict.
    """
...
class SafeConsumptionAuditor:
    ...
    def __init__(
        ...
        *,
        ...
        timeout      : Per-request timeout (read) in seconds.
        rate_limit   : Minimum spacing between requests to a single host.
        log_monitor  : Callback invoked for each finding (sink). Signature: Callable[[dict], None]
        """
        ...
        print(
            ...
            f"(timeout={self.timeout}s, rate_limit={self.rate_limit}s, per_host={self.per_host_max_concurrency})"
        )
    ...
    def _create_secure_session() -> requests.Session:
        """
        Build a tuned requests.Session with:
        - retry policy for transient 5xx
        - large connection pool (keep-alive)
        - optional HTTP/2 support (default OFF)
            Enable via: os.environ["APISCAN_HTTP2"]="1" or module var APISCAN_HTTP2 = 1
        """
        import os
        import requests
        from requests.adapters import HTTPAdapter
        ...
        # Retry only on transient server errors; small backoff for speed
        retries = Retry(
            total=2,
            connect=2,
            read=2,
        ...
        # Big pool + block when exhausted (prevents bursty thread failures)
        adapter = HTTPAdapter(
            max_retries=retries,
        ...
        s.headers.update(
            {
                "User-Agent": "safe_consumption10/10",
    ...
    def _throttle(self, domain: str) -> None:
        """Allow up to N concurrent calls per host and enforce min spacing."""
    ...
    def _is_parse_error(self, response: requests.Response) -> bool:
        ...
        return any(p in body for p in self.JSON_PARSE_ERRORS) or (
            "application/json" in ctype and response.status_code == 400
        )
    ...
    def classify_transport_anomaly(self, url: str, method: str, exc: Exception | None, elapsed: float) -> str:
        """
        Returns severity label for transport-level anomalies (smuggling/TE-CL etc.).
        Only escalate if we saw differential behavior between CL:TE vs TE:CL or
        intermediaries returned 400/502 with known parser errors.
        """
    ...
    def _dedupe_issues(self) -> None:
        """
        Keep only one finding per unique (method, path, status, severity, issue, payload).
        Count duplicates.
        """
    ...
    def _test_injection(
        ...
        *,
        ...
        Supports: SQL, NoSQL, XSS, SSTI, SSRF, LDAP, XXE, CMDI, JSONP, PATH, CRLF, HPP, GraphQL
        """
        ...
        try:
            ...
            if final_method == "GET":
                ...
                response = self.session.get(attack_url, timeout=(3, self.timeout), allow_redirects=False)
                requested_url = attack_url
            else:
                base_url = test_url.split("?", 1)[0]
                response = self.session.post(
                    base_url,
                    data={"input": payload},
                    timeout=(3, max(self.timeout, 10)),
                    allow_redirects=False,
                )
                requested_url = base_url + "?input=" + urlparse.quote_plus(payload)

            if self._is_injection_successful(response, attack_type, payload=payload):
                self._log(
        ...
        try:
            ...
            for host in host_payloads:
                ...
                try:
                    ...
                    if ("password" in ep_low and "reset" in ep_low) and (
                        host in r.headers.get("Location", "") or host.lower() in (r.text or "").lower()
                    ):
                        self._log(
    ...
    def _is_injection_successful(
        ...
        *,
        ...
        if attack_type == "nosql":
            specific = any(re.search(p, low) for p in (r"mongo.*error", r"mongodb.*error", r"bson.*error"))
        ...
        if attack_type == "ssti":
            specific = any(re.search(p, low) for p in (
                r"template.*syntax.*error", r"jinja2.*exception", r"django.*template.*error",
                r"twig.*error", r"freemarker", r"mustache", r"thymeleaf"
            ))
        ...
        if attack_type == "ssrf":
            meta_hits = any(ind in low for ind in (
                "/latest/meta-data", "169.254.169.254", "instance-id", "ami-id", "availability-zone",
                "computemetadata", "metadata.google.internal", "project-id"
            ))
            return meta_hits

        if attack_type == "ldap":
            specific = any(re.search(p, low) for p in (r"ldap.*error.*\d+", r"invalid.*credentials", r"bind.*failed"))
            return specific or any(k in low for k in self.LDAP_ERROR_KEYWORDS) or self._has_engine_marker(low, "ldap")

        if attack_type == "xxe":
            specific = any(re.search(p, low) for p in (r"xml.*parsing.*error", r"entity.*reference", r"doctype.*not.*allowed"))
    ...
    def _is_false_positive(self, response: requests.Response) -> bool:
        ...
        waf_patterns = (r"cloudflare", r"akamaighost", r"imperva", r"barracuda", r"fortinet")
        ...
        try:
            ...
            if r.status_code >= 400:
                ...
                if any(k in body for k in self.SQL_ERROR_KEYWORDS) or any(rx.search(body) for rx in self.SQL_ERROR_RX):
                    ...
                    self._log("Possible NOSQL injection", endpoint, "Critical", response=r)
                else:
                    self._log("Basic security fail", endpoint, "Info", response=r)

        except Exception as exc:
            self._log("Basic security test failed", endpoint, "Info", extra={"error": str(exc)})

    def _test_crlf_injection(self, endpoint: str) -> None:
        """
        CRLF response-splitting probe (strong signals only)
        """
        if stop_requested.is_set():
            return
        ...
        try:
            ...
            base_variants = [
                f"%0d%0a{canary_name}: {canary_val}",
                f"%0d%0aSet-Cookie: {canary_cookie}",
                f"%0d%0aLocation: {canary_loc}",
                f"%0d%0aX-Accel-Redirect: {canary_path}",
            ]
                    ...
                    try:
                        ...
                        if canary_name.lower() in hdrs_dict and canary_val in hdrs_dict[canary_name.lower()]:
                            self._log(
                                ...
                                payload=inj,
                                response=r,
                                extra={"param": param, "injected_header": canary_name, "value": canary_val},
                            )
                            continue
                        ...
                        if loc and canary_val in loc:
                            self._log(
                                "CRLF injection (Location header poisoned)",
                                test_url,
                                "High",
                                payload=inj,
                                response=r,
                                extra={"param": param, "location": loc},
                            )
                            continue
                        ...
                        if any(canary_name.lower() in v.lower() and canary_val in v for v in set_cookie_lines):
                            self._log(
                                "CRLF injection (Set-Cookie poisoned)",
                                test_url,
                                "High",
                                payload=inj,
                                response=r,
                                extra={"param": param, "set_cookie": set_cookie_lines[:2]},
                            )
                            continue

                        if f"{canary_name.lower()}: {canary_val.lower()}" in body_low:
                            self._log(
                                "CRLF header string reflected in body",
                                test_url,
                                "Info",
                                payload=inj,
                                response=r,
                        ...
                        self._log(
                            ...
                            test_url,
                            sev,
                            extra={"error": str(e), "param": param, "payload": inj},
                        )
                ...
                for payload in ssrf_payloads:
                    ...
                    try:
                        ...
                        get_response = self.session.get(
                            endpoint, params={param: payload}, timeout=(3, max(self.timeout, 10)), allow_redirects=False
                        )

                        self._throttle(domain)
                        post_response = self.session.post(
                            endpoint, data={param: payload}, timeout=(3, max(self.timeout, 10)), allow_redirects=False
                        )
                        ...
                        self._log(
                            "SSRF test failed", endpoint, "Info", extra={"error": str(e), "param": param, "payload": payload}
                        )
    ...
    def _is_ssrf_successful(self, response: requests.Response, payload: str) -> bool:
        """Detect successful SSRF exploitation with low false positives."""
        content = (response.text or "").lower()
    ...
    def _test_hpp(self, endpoint: str) -> None:
        """
        HTTP Parameter Pollution test with improved detection
        """
        if stop_requested.is_set():
            return
        try:
            ...
            for param in self.HPP_PARAMS:
                ...
                try:
                    ...
                    param_reflected = any(
                        [
                            f"{param}=1" in body and f"{param}=2" in body,
                            f"{param}=1,2" in body,
                            f"{param}=2,1" in body,
                            f"{param}[]=1" in body and f"{param}[]=2" in body,
                        ]
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as main_executor:
                ...
                for ep in reachable_endpoints:
                    ...
                    test_fns = [
                        ...
                        partial(self._test_header_manipulation, ep),
...
#---------------------- has_sql_evidence 
    ...
    def _run_injection_tests_parallel(self, endpoint: str, test_type: str) -> None:
        ...
        test_urls: List[Tuple[str, str, str]] = []  # (url, method, payload)
    ...
    def _looks_like_secret(self, text: str) -> bool:
        ...
        Attempts to avoid flagging plain words like 'token' without entropy.
        """
        if not text:
            return False
        t = text.strip()

        b64 = re.findall(r"[A-Za-z0-9+/=]{24,}", t)
        hx = re.findall(r"\b[0-9a-fA-F]{32,}\b", t)
