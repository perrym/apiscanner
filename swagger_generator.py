...
JSON_CT_HINTS = ('application/json', 'application/problem+json', 'application/vnd.api+json', 'application/x-ndjson', 'text/json')
API_CT_HINTS = JSON_CT_HINTS + ('application/xml', 'text/xml')
...
class UltimateSwaggerGenerator:
    ...
    def __init__(self, base_url: str, delay: float=0.0, aggressive: bool=False):
        ...
        self.session.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:138.0) Gecko/20100101 Firefox/140.0', 'Accept': 'application/json, text/html, application/xml;q=0.9, */*;q=0.8', 'Accept-Encoding': 'gzip, deflate'})
        self.swagger = {'openapi': '3.0.0', 'info': {'title': f'API Documentation for {base_url}', 'version': '1.0.0'}, 'servers': [{'url': base_url}], 'paths': {}, 'components': {'schemas': {}, 'securitySchemes': {'bearerAuth': {'type': 'http', 'scheme': 'bearer', 'bearerFormat': 'JWT'}}}}
        self.visited: Set[str] = set()
        self.lock = threading.Lock()
        self.api_patterns = ['(?i)/api/', '(?i)/v[0-9]+/', '(?i)\\.(json|xml)$', '(?i)/graphql', '(?i)/rest/', '(?i)/restapi/', '(?i)/service(s)?/', '(?i)/data/', '(?i)/swagger', '(?i)/openapi', '(?i)/endpoints?', '(?i)/rpc/', '(?i)/jsonrpc', '(?i)/soap/', '(?i)/ws/', '(?i)/wss/', '(?i)/socket\\.io', '(?i)/sockjs', '(?i)/_ah/api', '(?i)/_api', '(?i)/ajax/', '(?i)/async/', '(?i)/backend/', '(?i)/v2/', '(?i)/v3/']
        self.auth_tokens = {}
        self.login_url = None
    ...
    #================funtion _mark_visited _mark_visited =============
    ...
    #================funtion _normalise_path _normalise_path =============
    ...
    #================funtion _update_swagger_security_schemes _update_swagger_security_schemes =============
    def _update_swagger_security_schemes(self):
        if isinstance(self.session.auth, HTTPBasicAuth):
            self.swagger['components']['securitySchemes']['basicAuth'] = {'type': 'http', 'scheme': 'basic'}
        for header in self.session.headers:
            if header.lower() in ['x-api-key', 'api-key']:
                self.swagger['components']['securitySchemes']['apiKeyAuth'] = {'type': 'apiKey', 'in': 'header', 'name': header}
    ...
    #================funtion set_token_auth set_token_auth =============
    ...
    #================funtion set_custom_header set_custom_header =============
    ...
    #================funtion set_login_form set_login_form =============
    def set_login_form(self, url: str, data: Dict[str, str]):
        self.login_url = url
        self.login_data = data

    #================funtion authenticate authenticate =============
    ...
    #================funtion _handle_rate_limiting _handle_rate_limiting =============
    ...
    #================funtion _should_crawl _should_crawl =============
    ...
    #================funtion _crawl_page _crawl_page =============
    ...
    #================funtion _check_common_headers _check_common_headers =============
    def _check_common_headers(self):
        headers_to_check = [('Accept', 'application/json'), ('X-Requested-With', 'XMLHttpRequest'), ('Content-Type', 'application/json')]
    ...
    #================funtion _classify_endpoint _classify_endpoint =============
    ...
    #================funtion _looks_like_api _looks_like_api =============
    ...
    #================funtion _is_api_endpoint _is_api_endpoint =============
    def _is_api_endpoint(self, url: str) -> bool:
        path = urlparse(url).path.lower()
        query = urlparse(url).query.lower()
        if any((re.search(pattern, path) for pattern in self.api_patterns)):
            return True
        return 'api' in path or path.endswith('.json') or 'json' in path or ('xml' in path) or ('data' in path) or ('v1' in path) or ('v2' in path) or ('graphql' in path) or ('token=' in query) or ('auth=' in query) or ('bearer' in self.session.headers.get('Authorization', '').lower()) or any((param in query for param in ['format=json', 'type=api', 'output=json']))

    #================funtion _discover_methods _discover_methods =============
    ...
    #================funtion _process_possible_api _process_possible_api =============
    def _process_possible_api(self, url: str):
        ...
        if not self._mark_visited(url):
            return
        if url.lower().endswith(('swagger', 'swagger.json', 'swagger.yaml', 'openapi', 'openapi.json', 'openapi.yaml')):
        ...
        try:
            ...
            with open(self.log_path, 'a', encoding='utf-8') as log:
                log.write(json.dumps({'url': url, 'path': clean_path, 'status': resp.status_code, 'ct': resp.headers.get('Content-Type', ''), 'classification': classification}) + '\n')
    ...
    #================funtion _ensure_operation_exists _ensure_operation_exists =============
    ...
    #================funtion _process_api_response _process_api_response =============
    def _process_api_response(self, url: str, response: requests.Response, method: str='get'):
        ...
        operation['responses'][str(response.status_code)] = {'description': 'Auto-discovered response', 'content': {ct: {'schema': schema}}}

    #================funtion _generate_schema _generate_schema =============
    ...
    #================funtion _process_form _process_form =============
    ...
    #================funtion _find_websockets _find_websockets =============
    def _find_websockets(self, text: str, base_url: str):
        patterns = ['new\\s+WebSocket\\(["\\\'](wss?://[^"\\\']+)["\\\']', '\\.connect\\(["\\\'](wss?://[^"\\\']+)["\\\']', 'socket\\.io\\(["\\\']([^"\\\']+)["\\\']']
        for pat in patterns:
    ...
    #================funtion _find_graphql _find_graphql =============
    def _find_graphql(self, text: str, base_url: str):
        patterns = ['graphqlUrl:\\s*["\\\']([^"\\\']+)["\\\']', 'uri:\\s*["\\\']([^"\\\']+/graphql)["\\\']', 'fetch\\(["\\\']([^"\\\']+/graphql)["\\\']']
        for pat in patterns:
            for m in re.finditer(pat, text, re.DOTALL | re.IGNORECASE):
    ...
    #================funtion _should_inspect _should_inspect =============
    ...
    #================funtion _prune_non_json_paths _prune_non_json_paths =============
    ...
    #================funtion save_swagger save_swagger =============
...
#================funtion configure_authentication configure_authentication =============
def configure_authentication(args) -> requests.Session:
    ...
    if getattr(args, 'flow', None) == 'client' and args.client_id and args.client_secret and args.token_url:
        try:
            ...
            token = oauth.fetch_token(token_url=args.token_url, client_id=args.client_id, client_secret=args.client_secret)
            sess = oauth
